 ...
 * {@link PersistenceExceptionTranslationInterceptor} as well as an annotation based {@link
 * TransactionInterceptor} to the proxy.
 *
 * @author Oliver Gierke
 */
class OneDataTransactionalRepositoryProxyPostProcessor implements RepositoryProxyPostProcessor {
  ...
  /**
   * Creates a new {@link TransactionalRepositoryProxyPostProcessor} using the given {@link
   * ListableBeanFactory} and transaction manager bean name.
   *
   * @param beanFactory            must not be {@literal null}.
   * @param transactionManagerName must not be {@literal null} or empty.
   */
  public OneDataTransactionalRepositoryProxyPostProcessor(
  ...
  public void postProcess(ProxyFactory factory, RepositoryInformation repositoryInformation) {
    ...
    TransactionInterceptor
        transactionInterceptor =
        new TransactionInterceptor(null, transactionAttributeSource);
   ...
   * interface for working with transaction metadata in JDK 1.5+ annotation format. <p> This class
   * reads Spring's JDK 1.5+ {@link Transactional} annotation and exposes corresponding transaction
   * attributes to Spring's transaction infrastructure. Also supports JTA 1.2's and EJB3's {@link
   * javax.ejb.TransactionAttribute} annotation (if present). This class may also serve as base
   * class for a custom TransactionAttributeSource, or get customized through {@link
  ...
  static class CustomAnnotationTransactionAttributeSource
    ...
    /**
     * Create a default CustomAnnotationTransactionAttributeSource, supporting public methods that
     * carry the {@code Transactional} annotation or the EJB3 {@link javax.ejb.TransactionAttribute}
     * annotation.
     */
    ...
    /**
     * Create a custom CustomAnnotationTransactionAttributeSource, supporting public methods that
     * carry the {@code Transactional} annotation or the EJB3 {@link javax.ejb.TransactionAttribute}
     * annotation.
     *
     * @param publicMethodsOnly whether to support public methods that carry the {@code
     *                          Transactional} annotation only (typically for use with proxy-based
     *                          AOP), or protected/private methods as well (typically used with
     ...
     */
    public CustomAnnotationTransactionAttributeSource(
        TransactionAnnotationParser annotationParser) {
     ...
     */
    public CustomAnnotationTransactionAttributeSource(
        TransactionAnnotationParser... annotationParsers) {
      this.publicMethodsOnly = true;
    ...
    /**
     * Determine the transaction attribute for the given method or class. <p> This implementation
     * delegates to configured {@link TransactionAnnotationParser TransactionAnnotationParsers} for
     * parsing known annotations into Spring's metadata attribute class. Returns {@code null} if
     * it's not transactional. <p> Can be overridden to support custom annotations that carry
     * transaction metadata.
     *
     * @param ae the annotated method or class
     * @return TransactionAttribute the configured transaction attribute, or {@code null} if none
     * was found
     */
    ...
    public boolean equals(Object other) {
      ...
      CustomAnnotationTransactionAttributeSource
          otherTas =
          (CustomAnnotationTransactionAttributeSource) other;
  ...
  /**
   * Abstract implementation of {@link TransactionAttributeSource} that caches attributes for
   * methods and implements a fallback policy: 1. specific target method; 2. target class; 3.
   * declaring method; 4. declaring class/interface. <p> Defaults to using the target class's
   * transaction attribute if none is associated with the target method. Any transaction attribute
   * associated with the target method completely overrides a class transaction attribute. If none
   * found on the target class, the interface that the invoked method has been called through (in
   * case of a JDK proxy) will be checked. <p> This implementation caches attributes by method after
   * they are first used. If it is ever desirable to allow dynamic changing of transaction
   * attributes (which is very unlikely), caching could be made configurable. Caching is desirable
   ...
   * @since 1.1
   */
  abstract static class AbstractFallbackTransactionAttributeSource
     ...
     */
    private final static TransactionAttribute
        NULL_TRANSACTION_ATTRIBUTE =
        new DefaultTransactionAttribute();

    /**
     * Logger available to subclasses. <p> As this base class is not marked Serializable, the logger
     * will be recreated after serialization - provided that the concrete subclass is Serializable.
     */
    protected final Logger logger = LoggerFactory.getLogger(getClass());

    /**
     * Cache of TransactionAttributes, keyed by DefaultCacheKey (Method + target Class). <p> As this
     * base class is not marked Serializable, the cache will be recreated after serialization -
     * provided that the concrete subclass is Serializable.
     */
    final Map<Object, TransactionAttribute>
        attributeCache =
        new ConcurrentHashMap<Object, TransactionAttribute>();
    ...
    /**
     * Determine the transaction attribute for this method invocation. <p> Defaults to the class's
     * transaction attribute if no method attribute is found.
     *
    ...
    public TransactionAttribute getTransactionAttribute(Method method, Class<?> targetClass) {
      ...
      if (cached != null) {
        ...
        if (txAtt == null) {
          txAtt = computeOneDataTransactionAttribute(method);
        }
        // Put it in the cache.
        if (txAtt == null) {
    ...
    /**
     * Determine a cache key for the given method and target class. <p> Must not produce same key
     * for overloaded methods. Must produce same key for different instances of the same method.
     *
    ...
    // todo jade support
    private TransactionAttribute computeOneDataTransactionAttribute(Method method) {
      TransactionAttribute txAtt = null;
      // We need to work it out.
      ...
      if (sqlAnnotation != null) {
        ...
        try {
          ...
          switch (sqlType) {
            ...
            case PROCEDURE:
              tempMethod = tempClass.getDeclaredMethod(
                  "call", String.class, Object[].class, RowMapperFactory.class,
                  ResultSetProcedureResult[].class);
              break;
            default:
              tempMethod = tempClass.getDeclaredMethod(
    ...
    /**
     * Should only public methods be allowed to have transactional semantics? <p> The default
    ...
    private static class DefaultCacheKey {
      ...
      @Override
      public int hashCode() {
        return this.method.hashCode() * 29 + (this.targetClass != null ? this.targetClass.hashCode()
                                                                       : 0);
      }
    }
  }
